...
{
    [Route("api/procurementManagement/PO/{poId:long}/poInspectionComment")]
    [Authorize]
    [Raybod.SCM.Utility.Security.Authorize(0)]
    [ApiController]
    [SwaggerArea(AreaName = "procurementManagement")]
    public class POInspectionCommentController : ControllerBase
    {
        private readonly IPOCommentService _poCommentService;
        private readonly IUserService _userService;
        private readonly ILogger<POInspectionCommentController> _logger;

        public POInspectionCommentController(
            IPOCommentService poCommentService,
            IUserService userService,
            ILogger<POInspectionCommentController> logger,IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<object> GetPOCommentAsync(long poId, [FromQuery] POCommentQueryDto query)
        {
            ...
            var serviceResult = await _poCommentService.GetPOCommentAsync(authenticate, poId, Domain.Enum.PoCommentType.Inspection, query);
        ...
        public async Task<object> AddPOCommentAsync(long poId, [FromBody] AddPOCommentDto model)
        {
            ...
            _logger.LogWarning(logInformation.InformationText, logInformation.Args);
            var serviceResult = await _poCommentService.AddPOCommentAsync(authenticate, poId, Domain.Enum.PoCommentType.Inspection, model);
            return serviceResult.ToWebApiResultVCore(authenticate.language);
        }
        [HttpGet]
        ...
        public async Task<object> DownloadPOCommentAttachmentAsync(long poId, long commentId, string fileSrc)
        {
            ...
            _logger.LogWarning(logInformation.InformationText, logInformation.Args);
            var streamResult = await _poCommentService.DownloadPOCommentAttachmentAsync(authenticate, poId, commentId, Domain.Enum.PoCommentType.Inspection, fileSrc);
