...
{
    [Route("api/procurementManagement/PO/{poId:long}/poSupplierDocumentComment")]
    [Authorize]
    [Raybod.SCM.Utility.Security.Authorize(0)]
    [ApiController]
    [SwaggerArea(AreaName = "procurementManagement")]
    public class POSupplierDocumentCommentController : ControllerBase
    {
        private readonly IPOCommentService _poCommentService;
        private readonly IUserService _userService;
        private readonly ILogger<POSupplierDocumentCommentController> _logger;

        public POSupplierDocumentCommentController(
            IPOCommentService poCommentService,
            IUserService userService,
            ILogger<POSupplierDocumentCommentController> logger,IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<object> GetPOCommentAsync(long poId, [FromQuery] POCommentQueryDto query)
        {
            ...
            var serviceResult = await _poCommentService.GetPOCommentAsync(authenticate, poId, PoCommentType.SupplierDocument, query);
        ...
        public async Task<object> AddPOCommentAsync(long poId, [FromBody] AddPOCommentDto model)
        {
            ...
            _logger.LogWarning(logInformation.InformationText, logInformation.Args);
            var serviceResult = await _poCommentService.AddPOCommentAsync(authenticate, poId, PoCommentType.SupplierDocument, model);
            return serviceResult.ToWebApiResultVCore(authenticate.language);
        }
        [HttpGet]
        ...
        public async Task<object> DownloadPOCommentAttachmentAsync(long poId, long commentId, string fileSrc)
        {
            ...
            _logger.LogWarning(logInformation.InformationText, logInformation.Args);
            var streamResult = await _poCommentService.DownloadPOCommentAttachmentAsync(authenticate, poId, commentId, PoCommentType.SupplierDocument, fileSrc);
